...
const mockInitialData = {
    ...
    deletedAt: null,
    subTasks: [
        { id: 's1', taskId: '1', title: 'Sub 1', completedAt: null, sortOrder: 0, createdAt: '' }
    ]
}
...
describe('TaskForm', () => {
    ...
    it('renders with initial data for editing', () => {
        render(<TaskForm initialData={mockInitialData as any} onSubmit={mockOnSubmit} onCancel={mockOnCancel} />)
        expect(screen.getByPlaceholderText('What needs to be done?')).toHaveValue('Original Title')
    ...
    it('auto-completes parent when all sub-tasks are checked (BR-086)', async () => {
        ...
        await waitFor(() => {
            const parentTitleInput = screen.getByPlaceholderText('What needs to be done?')
    ...
    it('prompts for scope when editing a recurring task (BR-101)', async () => {
        ...
        render(<TaskForm initialData={recurringTask as any} onSubmit={mockOnSubmit} onCancel={mockOnCancel} />)

        // Change title to make it dirty if needed, or just submit
        fireEvent.change(screen.getByPlaceholderText('What needs to be done?'), { target: { value: 'Updated' } })
