...
vi.mock('@/components/mood/MoodCheckinModal', () => ({
    MoodCheckinModal: ({ isOpen, onSelect }: any) => isOpen ? (
...
describe('Habit Integration Flow', () => {
    ...
    beforeEach(() => {
        ...
        mockFetch.mockImplementation((url) => {
                    ...
                    json: async () => ([
                        {
                            ...
                            isCompleted: false,
                            categoryId: 'cat-1',
                            category: { id: 'cat-1', name: 'Health', icon: '💧', sortOrder: 1 }
                        }
                    ])
                })
            }
            ...
            if (url.includes('/api/completions/dates')) return Promise.resolve({ ok: true, json: async () => ({ dates: [] }) })
            if (url.includes('/complete')) return Promise.resolve({ ok: true, json: async () => ({ success: true }) })
            if (url.includes('/api/mood-checkins')) return Promise.resolve({ ok: true, json: async () => ({ success: true }) })
    ...
    it('should open mood modal when completing a habit', async () => {
        ...
        await waitFor(() => {
            expect(mockFetch).toHaveBeenCalledWith(
                ...
                expect.objectContaining({ method: 'POST', body: expect.stringContaining('HAPPY') })
            )
        }, { timeout: 3000 })
    })
})
