...
import { calculateNextDueDate, processTaskRecurrence } from '@/lib/services/recurrence'
...
describe('Recurrence Service', () => {
    describe('calculateNextDueDate', () => {
        ...
        it('should handle CUSTOM recurrence (every 3 days)', () => {
            const current = new Date('2025-02-10')
            const next = calculateNextDueDate(current, Recurrence.CUSTOM, { interval: 3, unit: 'days' })
            expect(next.toISOString()).toContain('2025-02-13')
        })

        it('should handle CUSTOM recurrence (every 2 weeks)', () => {
            const current = new Date('2025-02-10')
            const next = calculateNextDueDate(current, Recurrence.CUSTOM, { interval: 2, unit: 'weeks' })
