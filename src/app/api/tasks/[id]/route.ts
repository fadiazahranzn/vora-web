    ...
    try {
        ...
        const updatedTask = await prisma.$transaction(async (tx) => {
            ...
            await tx.task.update({
                ...
                data: {
                    ...taskData,
                    dueDate: taskData.dueDate === null ? null : taskData.dueDate ? new Date(taskData.dueDate) : undefined,
                    completedAt: taskData.completedAt === null ? null : taskData.completedAt ? new Date(taskData.completedAt) : undefined,
                },
            });

            // Handle recurrence logic (BR-097)
            const isPreviouslyCompleted = !!existingTask.completedAt;
            const isNowCompleted = taskData.completedAt !== undefined && taskData.completedAt !== null;
            ...
            if (subTasks) {
                ...
                const keepIds = subTasks.filter((st: any) => st.id).map((st: any) => st.id);
