...
export async function GET(req: NextRequest) {
    try {
        ...
        if (view === 'yearly') {
            ...
            dataPoints = months.map(d => ({ date: d, label: format(d, 'MMM yyyy') }));
            ...
            if (view === 'yearly') {
                ...
                daysInMonth.forEach(day => {
                    ...
                    const dayCompletions = completions.filter(c =>
                        format(c.date, "yyyy-MM-dd") === dayString
                    );
                ...
                const habitsDue = habits.filter(habit => {
                    ...
                    if (habit.frequency === 'DAILY') return true;
                    if (habit.frequency === 'WEEKLY') {
                        return habit.weeklyDays.includes(getDay(pointDate));
                    }
                    if (habit.frequency === 'MONTHLY') {
                        return habit.monthlyDates.includes(getDate(pointDate));
                    }
                    return false;
                });
                ...
                const dayCompletions = completions.filter(c =>
                    format(c.date, "yyyy-MM-dd") === pointDateString
                );
                totalCompleted = dayCompletions.length;
            }

            // Avoid division by zero
            const percentage = totalDue === 0 ? 0 : Math.round((totalCompleted / totalDue) * 100);
