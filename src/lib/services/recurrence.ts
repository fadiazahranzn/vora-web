 ...
 */
export function calculateNextDueDate(currentDueDate: Date, type: Recurrence, rule?: any): Date {
    switch (type) {
        ...
        case Recurrence.CUSTOM:
            if (rule && typeof rule === 'object') {
                ...
                switch (unit) {
                    case 'days': return addDays(currentDueDate, interval)
                    case 'weeks': return addWeeks(currentDueDate, interval)
                    case 'months': return addMonths(currentDueDate, interval)
                }
            }
...
export async function processTaskRecurrence(taskId: string, tx: any) {
    ...
    const nextDueDate = calculateNextDueDate(currentDueDate, task.recurrence, task.recurrenceRule)
