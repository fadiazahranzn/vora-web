...
export const TaskForm: React.FC<TaskFormProps> = ({
    ...
    const form = useForm<TaskFormData>({
        ...
        defaultValues: {
            ...
            recurrenceRule: initialData?.recurrenceRule ? JSON.stringify(initialData.recurrenceRule) : '',
    ...
    const handleScopeSelection = async (scope: 'this' | 'future') => {
        ...
        const payload = {
            ...
            dueDate: pendingData.dueDate ? format(pendingData.dueDate, 'yyyy-MM-dd') : null,
            recurrenceRule: pendingData.recurrence === Recurrence.CUSTOM && pendingData.recurrenceRule
    ...
    return (
        <form onSubmit={handleSubmit(handleFormSubmit)} className={styles.form}>
                ...
                <div className={styles.promptOverlay}>
                    <div className={styles.promptContent}>
                        ...
                        <p className={styles.promptText}>
                            This is a repeating task. How would you like to apply your changes?
                        </p>
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <div
                                onClick={() => setValue('completedAt', completedAt ? null : new Date().toISOString(), { shouldDirty: true })}
                                style={{ cursor: 'pointer', color: completedAt ? 'var(--primary)' : 'var(--text-tertiary)' }}
                            >
                                {completedAt ? <CheckCircle2 size={24} /> : <Circle size={24} />}
                            </div>
                            <Input
                                label=""
            ...
            {/* Sub-tasks */}
            <div className={styles.fieldGroup}>
                <label className={styles.label}>Sub-tasks {fields.length > 0 && `(${fields.filter(f => !!(f as any).completedAt).length}/${fields.length})`}</label>
                                ...
                                <Controller
                                    ...
                                    render={({ field: inputField }) => (
                                        <Input
                                            ...
                                            style={{ textDecoration: isCompleted ? 'line-through' : 'none', color: isCompleted ? 'var(--text-tertiary)' : 'inherit' }}
                                            {...inputField}
                                        />
                                    )}
                                />
            ...
            <div className={styles.row}>
                        ...
                        render={({ field }) => (
                            <div className={styles.priorityGroup}>
                                {[Priority.HIGH, Priority.MEDIUM, Priority.LOW].map((p) => (
                                    ...
                                    >
                                        <div className={`${styles.priorityDot} ${styles[p.toLowerCase()]}`} />
                ...
                <div className={styles.col}>
                    ...
                    />
                    {errors.dueDate && (
                        <p className="vora-input__error">{errors.dueDate.message as string}</p>
                    )}
                </div>
            </div>
                        ...
                        render={({ field }) => (
                            <>
                                ...
                                {errors.recurrenceRule && (
                                    <p className="vora-input__error">{errors.recurrenceRule.message}</p>
                                )}
                            </>
                        )}
                    />
