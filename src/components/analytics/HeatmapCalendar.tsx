...
export function HeatmapCalendar() {
    ...
    const { data: detailData, isLoading: isLoadingDetail } = useHeatmapDetail(selectedDate);
    ...
    const calendarDays = useMemo(() => {
        ...
        return eachDayOfInterval({ start, end: startOfWeek(end, { weekStartsOn: 1 }) === end ? end : endOfMonth(currentMonth) });
        // Actually simpler to just get all days of interval from start of week of start of month to end of month
    }, [currentMonth]);
    ...
    return (
        <div className={styles.container}>
            <header className={styles.header}>
                <h3 className={styles.monthLabel}>{format(currentMonth, 'MMMM yyyy')}</h3>
            ...
            <div className={styles.grid}>
                ...
                {isLoading ? (
                    Array.from({ length: dateRange.length }).map((_, i) => (
                        <Skeleton key={`skeleton-${i}`} className={clsx(styles.dayCell, styles['rate-gray'])} />
                    ))
                ) : (
                    heatmapData?.map((day) => (
                        <button
                            key={day.date}
                            className={clsx(styles.dayCell, getRateColor(day.rate, day.scheduled))}
                        ...
                        >
                            ...
                            <div className={styles.tooltip}>
                                {format(parseISO(day.date), 'MMM d, yyyy')} — {day.rate}% ({day.completed}/{day.scheduled} habits)
                            </div>
                        </button>
                    ))
                )}
            ...
            <Modal
                ...
                title={selectedDate ? format(parseISO(selectedDate), 'EEEE, MMM d') : 'Day Details'}
            >
                {isLoadingDetail ? (
